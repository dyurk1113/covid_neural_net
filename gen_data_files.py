...
def build_CA_training_df(Reff_lag=21, min_date='2020-05-01', max_date='2020-11-31', use_cases=False, no_mon_col=False):
    ...
    if no_mon_col:
        header_row += ['no_mon']
    header_row += ['output_target']
    all_data = [header_row]
    for county in df['county'].unique():
        county_df = df.loc[df['county'] == county].dropna(subset=[rt_col])
        if len(county_df) == 0:
        ...
        for i in range(0, len(dates), 7):
            if dates[i] < min_date or dates[i] > max_date or i < Reff_lag + 7:
                continue
            if cases[i] - cases[i - 7] < 10 or cases[i] <= cases[i - 14]:
                continue
            if use_cases:
                output_target = np.log10((cases[i + 21] - cases[i + 7]) / (cases[i] - cases[i - 14]))
            else:
                output_target = (np.median(rt[i:i + 14]) - np.median(rt[i - 14:i]))
            log_case_rate = 0  # np.log10((cases[i] - cases[i - 14]) / tot_pop)  # Parameter no longer in use
            log_case_scaling = np.log10((cases[i + 21] - cases[i + 7]) / (cases[i] - cases[i - 14]))
            ...
            if no_mon_col:
                new_row += ['0']
            if use_cases:
                new_row += ['%.4f' % log_case_scaling]
            else:
                new_row += ['%.4f' % output_target]
